...
def run(filename=''):
	...
	count = 10
	  ...
	  if dict.has_key(key):
	    ...
	    #print word,
	    file_finished = open('script_compiled','a')
            global output
            output = word + ' '
...
file_finished = open('script_compiled','w')
for i in numpy.random.choice(22,1):  #total range is 0 to 23
    print 'script_' + str(i)
    filename =  '/Users/margaretlazarovits/Desktop/TAPS BA/script_' + str(i) + '.txt' #how to choose which file to run markov chain on
    script_i = run(filename)   
    file_finished.write('\n')         
    file_finished = open('script_compiled','a') #add script chunk
    file_finished.write('\n\n')
    #print 
